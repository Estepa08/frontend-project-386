...
describeIfDb("Admins API", () => {
  ...
  it("GET /api/admins — lists all admins", async () => {
    ...
    expect(res.body.some((a: any) => a.email === "testadmin@example.com")).toBe(true);
  });
  ...
  it("PATCH /api/admins/:id — updates admin", async () => {
    const res = await request.patch(`/api/admins/${adminId}`).send({
      name: "Updated Admin",
    });
    expect(res.status).toBe(200);
  ...
  it("DELETE /api/admins/:id — deletes admin", async () => {
    const res = await request.delete(`/api/admins/${adminId}`);
  ...
  describe("Meeting Types", () => {
    ...
    it("POST /api/admins/:id/meeting-types — creates meeting type", async () => {
      const res = await request.post(`/api/admins/${targetAdminId}/meeting-types`).send({
        duration: 30,
        category: "group",
      });
      expect(res.status).toBe(201);
